...
<body>
	...
	<section class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1 class="font-weight-bold text-danger">Quản lý phòng</h1>
				</div>
				<div class="col-sm-6">
	...
	<section class="content">
		...
		<div class="card-body">
			...
			<div class="card card-outline card-primary" style="padding: 20px;">
				...
				<hr>
				<form action="<c:url value='/admin/house/room'/>" id="formSubmit" method="get">
					<div class="col-xs-12">
									...
									<c:forEach var="item" items="${model.listResult}">
										<tr>
											<td><input type="checkbox" id="checkbox_${item.id}" value="${item.id}"
													class="checkAllItem"></td>
											<td>${item.roomName}</td>
											<td>
												<c:if test="${item.member > 0 }"><span class="text-red">Đang thuê</span>
												</c:if>
												<c:if test="${item.member == 0 }"><span class="text-green">Trống</span>
												</c:if>
											</td>
											<td>${item.member}</td>
											<td>${item.notes}</td>
											<td><a class="btn btn-sm btn-primary btn-edit" data-toggle="tooltip"
												...
												</a> <a class="btn btn-sm btn-primary btn-edit" data-toggle="tooltip"
													title="Xem phòng"
